        ...
        try {
            ...
            for (File file : expand) {
                ...
                if(fileName.contains("." + extOld)) {
                    // Создаем переменную с изображением нужного расширения и холст:
                    ...
                    // Рисуем квадрат:
                    g.drawRect((image.getWidth()-a)/2, (image.getHeight()-a)/2, a, a);

                    g.setColor(Color.BLACK); // задаем цвет текста
                    Font font = new Font("Arial", Font.BOLD|Font.ITALIC, 30); // создаем шрифт и его свойства
                    g.setFont(font); // устанавливаем созданный шрифт текущим
                    // Пишем текст:
                    g.drawString("Hello,", image.getWidth()/2 - 45,image.getHeight()/2);
                    g.drawString("World!", image.getWidth()/2 - 45,image.getHeight()/2 + 30);
